...
export default async function ListingDetailPage({ params }: { params: { id: string } }) {
  const listing = await prisma.listing.findUnique({
    ...
    include: {
      ...
      category: true, subCategory: true, fields: true,
      user: { select: { name: true, image: true, createdAt: true } },
    },
  })

  if (!listing || listing.status !== 'ACTIVE' || listing.expiresAt < new Date()) notFound()
  ...
  return (
    <>
      ...
      {/* Uses CSS class — 2-col on desktop, 1-col on mobile */}
      <div className="listing-detail-layout">
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: '0.75rem', flexWrap: 'wrap' }}>
            {isPremium && (
              <span style={{ background: 'var(--gold)', color: '#7B3F00', fontSize: '0.75rem', fontWeight: 700, padding: '3px 10px', borderRadius: 20, flexShrink: 0 }}>⭐ Featured</span>
            )}
            <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.3rem, 3vw, 1.9rem)', fontWeight: 900, color: 'var(--ink)', lineHeight: 1.2 }}>
              {listing.title}
            </h1>
          </div>

          {/* Price */}
          <div style={{ fontSize: 'clamp(1.25rem, 4vw, 1.6rem)', fontWeight: 800, color: 'var(--green)', marginBottom: '1.25rem' }}>
              ...
              {listing.images.length > 1 && (
                <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
                  {listing.images.slice(1).map((img, i) => (
                    <div key={img.id} style={{ position: 'relative', width: 90, height: 68, borderRadius: 8, overflow: 'hidden', flexShrink: 0 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))' }}>
                {listing.fields.map((f, i) => (
                  <div key={f.id} style={{ padding: '0.65rem 1rem', background: i % 2 === 0 ? '#fff' : 'var(--subtle)', borderBottom: i < listing.fields.length - 2 ? '1px solid var(--border)' : 'none' }}>
        ...
        <div>
          <div style={{
            ...
            <div style={{ background: isPremium ? 'linear-gradient(135deg, #7B4F00, #C9961E)' : 'var(--green)', padding: '1rem', textAlign: 'center' }}>
              <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.8)', marginBottom: 2 }}>Listed by</div>
              <div style={{ fontSize: '1rem', fontWeight: 700, color: '#fff' }}>{listing.contactName ?? listing.user.name ?? 'Seller'}</div>
            </div>
            <div style={{ padding: '1.25rem' }}>
              <ListingActions title={listing.title} phone={listing.contactPhone} email={listing.contactEmail} />
