...
export async function POST(req: Request) {
  ...
  const { title, description, price, priceLabel, parish, categoryId, subCategoryId, contactName, contactPhone, contactEmail, fields, imageUrls, tier } = body
  ...
  const listing = await prisma.listing.create({
    data: {
      ...
      status: 'ACTIVE',
      expiresAt,
      fields: { create: (fields ?? []).map((f: any) => ({ key: f.key, label: f.label, value: f.value })) },
